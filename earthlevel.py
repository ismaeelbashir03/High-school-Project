#createsa  function to be called when imported in 
def earth():
    ...
    import earthboss as b    #imports the boss class

    import earthball as f   #imports fireball class

    import earthblock as blk              #imports the block class

    import start            #imports start area

    import mysql.connector     #import my sql connector

    from warning import Warning_sign  # imports warning signs

    import file_handelling as fh        #imports file handelling file

    import insertion_sort as i      #imports insertion sort algorithm
    ...
    pygame.display.set_caption("earth level")       # Name your window
    ...
    bg = pygame.image.load("assets/earth/earthbg.jpg").convert()
    ...
    #defines a sql statement for geting element power and saving
    sql_earthsave = "UPDATE player SET earthunlock = 1, earthfinish = 1  WHERE active = 1"
    ...
    def redrawscreen(screen, bg, player):
    ...
    def opentilemap():
        ...
        file = open("assets/earth/earthleveltilemap.csv", "r")
    ...
    def drawinglevel(tilesize, music, x, y):
        ...
        #returns values
        return boss, player, tiles, blocks, block
    ...
    boss, player, tiles, blocks, block = drawinglevel(tilesize, music, x, y)
    ...
    while done == False and died == False:
        ...
        #calls the function to see if block needs to be changed
        boss.blockstate(block)

        #opens boss death check
        done = boss.checkdead()
        
        #calls the boss attack function
        fireballs, warnings = boss.attack(f.Fireball, fireballs, warnings, Warning_sign)

        #calls the function to kill warning
        ...
        for fireball in fireballs:
            ...
            fireball.kill(fireballs)
            


        # opens the draw screen function
        redrawscreen(screen, bg, player)
    ...
    if died == False:
        ...
        mycursor.execute(sql_earthsave)
